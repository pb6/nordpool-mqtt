...
def publish_price(topic, date):
    ...
    for row in data['data']['Rows']:
        for col in row['Columns']:
            ...
            publish.single(topic.format(value=hours_display), col['Value'].replace(',','.').replace(' ',''), hostname=mqtt_ip, port=mqtt_port, auth=auth)
            ...
            try:
                if date == dt_today and (datetime.time(int(hours[0]),00) <= now.time() <= datetime.time(int(hours[1]),00) or (datetime.time(23,00) <= now.time() and int(hours[1]) == 0)):
                    publish.single(topic.format(value='current'), col['Value'].replace(',','.').replace(' ',''), hostname=mqtt_ip, port=mqtt_port, auth=auth)
            except ValueError:
                pass
